  ...
  const [recipients] = await pool.query(
      `SELECT u.user_id, u.email
       ...
       WHERE u.role='student' AND (
           (SELECT course_id FROM events WHERE event_id=?) IS NULL OR
           EXISTS (
...
app.post('/api/events/:id/registrations', requireAuth, requireStudent, async (req, res) => {
  ...
  try {
    ...
    if (ev[0].course_id) {
      ...
      if (!enroll.length) return res.status(403).json({ error: 'Not enrolled in course' });
    }
...
app.get('/api/events/:id/attendance', requireAuth, async (req, res) => {
  const eventId = Number(req.params.id);
  try {
    const [[ev]] = await pool.query('SELECT organizer_id FROM events WHERE event_id = ?', [eventId]);
    if (!ev) return res.status(404).json({ error: 'Event not found.' });
    const isOwner = ev.organizer_id === req.user.id;
    if (!(req.user.role === 'admin' || isOwner)) return res.status(403).json({ error: 'Not authorized.' });
...
app.post('/api/events/:id/reminder', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!ev) return res.status(404).json({ error: 'Event not found.' });
    if (ev.status !== 'approved') return res.status(400).json({ error: 'Event must be approved first.' });
    if (!(req.user.role === 'admin' || ev.organizer_id === req.user.id)) return res.status(403).json({ error: 'Not authorized.' });

    if (!req.body.local) return res.status(400).json({ error: 'Provide "local": "YYYY-MM-DD HH:mm".' });
...
setInterval(async () => {
  try {
    ...
    for (const row of due) {
      ...
      await pool.query('UPDATE event_reminders SET is_sent = TRUE WHERE reminder_id = ?', [row.reminder_id]);
    }
